...
layout (std430, binding = 0) buffer TransformSBO {  Transform transforms[]; };
...
void main(){
    ...
    vec2 textureCoords[6] = {
        vec2(left, top), vec2(left, bottom), vec2(right, top),
        vec2(right, top), vec2(left, bottom), vec2(right, bottom)
    };
